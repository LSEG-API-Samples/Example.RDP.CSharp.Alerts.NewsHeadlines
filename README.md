# Using Refinitiv Data Platform Alerts with C#
...
This article demonstrates how to use C# to consume news headlines from RDP Alerts API. The RDP Alerts API allows users to setup subscriptions for different types of content, such as news, and research. The full code is available in [GitHub](https://github.com/Refinitiv-API-Samples/Example.RDP.CSharp.Alerts.NewsHeadlines).

## AutoRest
The RDP classed used in the article are generated by an AutoRest tool which is OpenAPI (f.k.a Swagger) Specification code generator. It supports C#, PowerShell, Go, Java, Node.js, TypeScript, Python, Ruby and PHP. You can follow steps in [How to use AutoRest with RDP](https://developers.refinitiv.com/article/how-use-autorest-rdp) to generate C#, Java, and Python code for RDP APIs.


The RDP packages used in this article are also available in the NuGet gallery, as shown below. You can use the newer versions of RDP APIs.

| NuGet Package|RDP API|
|--------------|-------|
...
This section demonstrates steps to develop a C# application to subscribe to news headlines from RDP alerts API. The application implements the workflow mentioned in the [Understanding the Alerts delivery mechanism in Refinitiv Data Platform](https://developers.refinitiv.com/article/alerts-delivery-mechanism-edp) article. It is developed with Visual Studio 2017 with .NET Core 2.0 and uses on the following NuGet packages:
...
There are six steps to consume news headlines from RDP alerts API. 
...
|--------|----------|
|Username|The username required to access RDP request-response API (typically email)|
...
```
This method accepts two parameters which are the **CredentialDetails** object used to connect to the Amazon Simple Queue Service (SQS) and the number of messages retrieved from the SQS. This method calls the **AmazonSQSClient.ReceiveMessageAsync** method to receive messages and returns the list of messages to the caller. Each retrieved message is also deleted from the SQS by the A**mazonSQSClient.DeleteMessageAsync** method. 


### 5. Decrypting messages

The messages in the queue are encrypted by the AES256 with GCM algorithm. To encrypt the messages, it requires the cryptography key in the **NewsSubscriptionDetails** object from the second step. For more information regarding how the messages are encrypted, please refer to the [Understanding the Alerts delivery mechanism in Refinitiv Data Platform](https://developers.refinitiv.com/article/alerts-delivery-mechanism-edp) article.
...
The full code is available in [GitHub](https://github.com/TR-API-Samples/Example.RDP.CSharp.Alerts.NewsHeadline).
...
```
## Summary
This article demonstrates how to use classes generated by AutoRest to authenticate and subscribe to news headlines from EDP Alert services. It uses the **AWSSDK.SQS** and **NSec.Cryptography** packages to retrieve the encrypted content from SQS and decrypt the content, respectively. The workflow used in the article is descriptively explained in the [Understanding the Alerts delivery mechanism in Refinitiv Data Platform](https://developers.refinitiv.com/article/alerts-delivery-mechanism-edp) article.


## References

1. [AutoRest](https://github.com/Azure/autorest)
2. [How to use AutoRest with RDP](https://developers.refinitiv.com/article/how-use-autorest-rdp)
3. [Understanding the Alerts delivery mechanism in Refinitiv Data Platform](https://developers.refinitiv.com/article/alerts-delivery-mechanism-edp)
4. [AWS SQS](https://docs.aws.amazon.com/sdk-for-net/v2/developer-guide/how-to-sqs.html)
5. [Example.RDP.CSharp.Alerts.NewsHeadlines](https://github.com/TR-API-Samples/Example.RDP.CSharp.Alerts.NewsHeadlines)
